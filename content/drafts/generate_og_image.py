...
Issue #4: "What's Dying While Everyone Watches Iran"
...
OUTPUT_PATH = "/Users/cooperanderson/projects/signal-dispatch/content/drafts/4-og-image.png"
...
TEXT_DIM  = (100, 98, 90)      # grid/scan line color (very dark)
...
bebas_ok   = download_font(BEBAS_URL, BEBAS_PATH)
jakarta_ok = download_font(JAKARTA_URL, JAKARTA_PATH)
...
GRID_STEP = 32
DOT_RADIUS = 1
DOT_ALPHA = 35   # very faint

# Build on a separate RGBA layer so we can composite at low opacity
...
for gx in range(0, WIDTH + GRID_STEP, GRID_STEP):
    for gy in range(0, HEIGHT + GRID_STEP, GRID_STEP):
        # Slight random offset per dot for organic feel
...
font_title_sm  = load_font(BEBAS_PATH,   108)
...
#    "── SIGNAL DISPATCH ──────────── (accent line)"
# ---------------------------------------------------------------------------
PUB_TEXT = "SIGNAL DISPATCH"
PUB_TRACKING = 5

# Short left dash before name
DASH_LEFT_W = 28
DASH_GAP    = 12
DASH_Y_OFFSET = 10   # vertically center dash on cap height
...
# ---------------------------------------------------------------------------
# 2. Main title: "What's Dying While / Everyone Watches Iran"
#    Two lines, Bebas Neue, large
# ---------------------------------------------------------------------------
TITLE_LINE1 = "What\u2019s Dying While"
TITLE_LINE2 = "Everyone Watches Iran"

# Measure both lines and pick appropriate size
# If line 2 is too wide at 130pt, the font handles it but let's check
bbox1 = font_title_lg.getbbox(TITLE_LINE1)
bbox2 = font_title_lg.getbbox(TITLE_LINE2)
...
if max(line1_w, line2_w) > max_title_w:
    # scale down
    scale = max_title_w / max(line1_w, line2_w)
    adjusted_size = int(130 * scale)
    font_title_use = load_font(BEBAS_PATH, adjusted_size)
else:
    font_title_use = font_title_lg

bbox1 = font_title_use.getbbox(TITLE_LINE1)
bbox2 = font_title_use.getbbox(TITLE_LINE2)
line_h = bbox1[3] - bbox1[1]

# Title line spacing
LINE_GAP = 8
draw.text((PADDING_X, TITLE_Y), TITLE_LINE1, font=font_title_use, fill=TEXT_PRI)
draw.text((PADDING_X, TITLE_Y + line_h + LINE_GAP), TITLE_LINE2, font=font_title_use, fill=TEXT_PRI)

# ---------------------------------------------------------------------------
# 3. Subtitle
# ---------------------------------------------------------------------------
SUBTITLE_TEXT = "Five institutional failures compounding under wartime distraction"
SUBTITLE_COLOR = (TEXT_SEC[0], TEXT_SEC[1], TEXT_SEC[2])

# Word wrap subtitle to ~900px wide
...
# ---------------------------------------------------------------------------
# 5. Metadata row: "DEEP DIVE | MARCH 16, 2026"
# ---------------------------------------------------------------------------
META_TEXT = "DEEP DIVE  |  MARCH 16, 2026"
draw_tracked_text(
    draw,
    (PADDING_X, META_Y),
    META_TEXT,
...
draw.line([(BAR_X, BAR_Y1), (BAR_X, BAR_Y2)], fill=(ACCENT[0], ACCENT[1], ACCENT[2], 60) if False else ACCENT, width=1)
# Use alpha via layer
...
# ---------------------------------------------------------------------------
# 7. Issue number watermark -- very faint top-right
# ---------------------------------------------------------------------------
